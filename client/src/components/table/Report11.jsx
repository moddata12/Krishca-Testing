...
function Report() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${BackUrl}/api/Report11Data`);
  ...
  const handleOkClick = async () => {
    try {
      ...
      const response = await fetch(
        `${BackUrl}/api/report11Date?start_date=${fromDate}&end_date=${toDate}`
      );
  ...
  const handleExportClick = async () => {
    try {
      ...
      const response = await fetch(`${BackUrl}/api/exportReport11`);
