...
function Report() {
  ...
  useEffect(() => {
    const fetchData = async () => {
      try {
        const response = await fetch(`${BackUrl}/api/Report1Data`);
  ...
  const handleOkClick = async () => {
    try {
      ...
      const response = await fetch(
        `${BackUrl}/api/report1Date?start_date=${fromDate}&end_date=${toDate}`
      );
  ...
  const handleExportClick = async () => {
    try {
      ...
      const response = await fetch(`${BackUrl}/api/exportReport1`);
